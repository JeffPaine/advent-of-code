...
func main() {
	...
	// Iterate over each claim in the fabric and find an entrire claim
	// where all its spots are exactly 1.
	for _, c := range claims {
		hasOverlaps := false
		for rowIdx, _ := range fab.rows {
			if hasOverlaps {
			...
			for spotIdx, _ := range fab.rows[rowIdx].spots {
				...
				// Now we're at the start of the spots we want to increment.
				// For the next c.width spots, increment them by one.
				if fab.rows[rowIdx].spots[spotIdx] > 1 {
					hasOverlaps = true
				}
			}
		}
		if !hasOverlaps {
			fmt.Println("Found claim with no overlaps: ", c.id)
		}
